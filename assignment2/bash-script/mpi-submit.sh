...
# WARNINGS: present configuration to collect information for 2^31  kpoints
...
do
	...
	do
		...
		do
			...
			do
				qsub -l nodes=1:ppn=${num_proc} -l walltime=2:00:00 -v PREC=${prec},DIMENSION=${n_dim},NUMP=${num_points},NUMPROC=${num_proc} -q dssc_gpu ./mpi-test.sh
			done
		done
	done
done
