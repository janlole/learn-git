...
{
	constexpr int max_depth_tested{16};
	constexpr int test_points{static_cast<int>(pow(2,24) - 1) };
	int best_depth{0};
		...
		for (auto try_depth{0}; try_depth < max_depth_tested; ++try_depth){
			...
			#pragma omp parallel 
			{
				#pragma omp single
				{
					#pragma omp task 
					{
						build_one_knode<core_algorithm>(&Grid[0], &Grid[test_points-1], &Nodes[0], 0, try_depth);
					}
				}
			}
			...
			// }
			depths_time[try_depth].coord[0] = try_depth;
			depths_time[try_depth].coord[1] = static_cast<float>(time);
		}
		auto t2_test = std::chrono::high_resolution_clock::now();
		auto time_test{std::chrono::duration_cast<std::chrono::milliseconds>(t2_test-t1_test).count()};

		std::cout << "TEST_time,TEST_points\n"
	...
	//_____________________________________________
	std::cout << "NDIM,NUMPOINTS,threads,OMP.TIME,depth,leaf_task" << std::endl;
