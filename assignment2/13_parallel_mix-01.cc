	...
	#pragma omp parallel  num_threads(4) 
		{
			#pragma omp single
			{
				...
				// std::cout << "THREADS\t" << threads << "\t" << rank << std::endl;
				#pragma omp task 
				{
					build_one_knode_omp<CORE_ALGORITHM>(first_kpoint, last_kpoint, working_node, axis+1, 3);
				}
			}
		}
	...
	if ( first_kpoint == last_kpoint){
		...
		if ( first_kpoint != mide ){
			...
			if ( depth < max_depth){
				...
				{
					build_one_knode_omp<choosen_algorithm>(first_kpoint, tmp, last_node, depth, max_depth);
				}
			}
			else{
				 #pragma omp task firstprivate(first_kpoint, tmp, last_node, depth)
		...
		if ( last_kpoint != mide ){
			...
			if ( depth < max_depth){
				...
				{
					build_one_knode_omp<choosen_algorithm>(tmp, last_kpoint, last_node, depth, max_depth);
				}
			}
			else{
				#pragma omp task firstprivate(last_kpoint, tmp, last_node, depth)
