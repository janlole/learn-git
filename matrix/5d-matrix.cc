...
int main(int argc, char *argv[])
{
	...
	int dim_dec{std::stoi(argv[1])};
	int test{std::stoi(argv[2])};
	int matr_test[12]{2400,100,100,1200,200,100,800,300,100,11,13,17};
	int dim_vect[MAX_DIM_DEC];
	dim_vect[0] = std::stoi(argv[2]);
	dim_vect[1] = std::stoi(argv[3]);
	dim_vect[2] = std::stoi(argv[4]);
	...
	for (auto i{0}; i < MAX_DIM_DEC; ++i)
		dims[i] = 1;
	for (auto i{MAX_DIM_DEC-1}; i > 3 - dim_dec -1; --i)
		dims[i] = 0;

	MPI_Dims_create(numproc, MAX_DIM_DEC, dims);						// MPI will decompose our process
	...
	// timing the pre-sending operations
	double end_pre_send;
	end_pre_send = MPI_Wtime() - start_pre_send;
	//__________________________________________________________________
	...
	double main_matrix;
	main_matrix = MPI_Wtime() - end_pre_send;
	//__________________________________________________________________
	...
	if (rank == root){
		...
		if (check_result){
			std::cout << "I am process " << rank 
					  << " and something went WRONG" << std::endl;
		}
		else{		
			std::cout << "I am process " << rank 
